...
- Manual smoke of the operator CLI path:
  - `python -m fullerene --event-type user_message --content "hello nexus" --state-dir state/.fullerene-state` (or omit `--state-dir` to use the same default)
  - Confirm `state/.fullerene-state/state.json` and `state/.fullerene-state/runtime-log.jsonl` were written.
