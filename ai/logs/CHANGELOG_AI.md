...
- 2026-05-06: Reworked `tests/test_executor.py` for Executor v1 registry/sandbox/file-op/approval/feedback coverage and added CLI manifest listing coverage in `tests/test_cli.py`.
- 2026-05-06: Implemented Verifier v1.5 tightening pass in `fullerene/verifier/artifacts.py` + `fullerene/verifier/checks.py`: extended Behavior v2.2 trace consistency validation, Context v2 packet validation, World Model v1 artifact/edge checks, output metadata + generic unsupported capability claim checks, skill-validator hook scaffolding, cross artifact policy/planner/executor consistency checks, and richer verifier summary metadata (`affected_artifact_ids`, `suggested_safe_decision`).
- 2026-05-06: Expanded `tests/test_verifier.py` with synthetic Verifier v1.5 coverage for invalid behavior final decision, ACT-policy mismatch, grounding/ambiguity warnings, Context v2 overload/missing working-memory warnings, World Model contradiction and self-link edge checks, unsupported capability claim detection, supported-claim pass path, malformed skill/executor outputs, live execution policy mismatch, and verifier metadata JSON-serializability.
- 2026-05-06: Updated harness docs for Verifier v1.5 scope and constraints in `ai/project/architecture.md`, `ai/knowledge/glossary.md`, and `ai/knowledge/decisions.md`.
- 2026-05-06: Refactored Behavior toward v2.2 modular architecture by adding `fullerene/behavior/__init__.py`, `signals.py`, `scoring.py`, `confidence.py`, `trace.py`, and `learning.py`; phrase heuristics remain isolated in `lexical.py` and structured signal objects in `models.py`.
- 2026-05-06: Updated `fullerene/facets/behavior.py` metadata contract with explicit `response_intent` while preserving `response_template` compatibility and existing `WAIT/RECORD/ASK/ACT` behavior contract.
- 2026-05-06: Expanded `tests/test_behavior.py` with Behavior v2.2 contract checks for lexical numeric extraction, scoring-from-signals, and response intent/template compatibility.
- 2026-05-06: Upgraded Behavior to v2.1 in `fullerene/facets/behavior.py` with deterministic conversational-intent routing, grounding-need/availability confidence, ambiguity-kind classification, richer confidence decomposition, score-adjustment trace fields, and expanded learning-event metadata while preserving `WAIT/RECORD/ASK/ACT` and Verifier-compatible trace structure.
- 2026-05-06: Expanded Behavior coverage in `tests/test_behavior.py` for source-request/follow-up/challenge/repeated-dissatisfaction/trace-field scenarios with synthetic metadata and no prompt-specific hardcoding.
- 2026-05-06: Updated harness docs for Behavior v2.1 semantics in `ai/project/architecture.md` and `ai/knowledge/glossary.md`.
- 2026-05-06: Upgraded World Model to v1 lifecycle semantics in `fullerene/world_model/models.py`, `fullerene/world_model/store.py`, and `fullerene/facets/world_model.py` with deterministic belief formation, normalized-key matching, Bayesian-style support/contradiction confidence updates, contradiction/redundancy status handling, provenance fields, and lightweight belief edges.
- 2026-05-06: Integrated world-model contradiction/uncertainty signals into LPB ingestion (`fullerene/signals/latent_pressure/buffer.py`) and exposed richer belief metadata in context assembly (`fullerene/context/assembler.py`).
- 2026-05-06: Added/updated tests for World Model v1 lifecycle and integrations in `tests/test_world_model.py`, `tests/test_latent_pressure.py`, and `tests/test_context.py`; full `test_*.py` suite passes after fixes.
# AI-facing changelog
